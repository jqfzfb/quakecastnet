...
class TimeSeriesDataSet(Dataset):
    ...
    * etc.
    ...
    def __init__(
        ...
        groups = self.data["groups"][0].clone()
    ...
    def _data_to_tensors(self, data: pd.DataFrame) -> Dict[str, torch.Tensor]:
        ...
        if "mask" in data.keys():
            mask = check_for_nonfinite(
                torch.tensor(data["mask"].to_numpy(dtype=np.float64), dtype=torch.float), "mask"
            ) 
        else:
            mask = None
        
        tensors = dict(
            reals=continuous, categoricals=categorical, groups=index, target=target, weight=weight, time=time, mask=mask,
        )
        return tensors
    ...
    def __getitem__(self, idx: int) -> Tuple[Dict[str, torch.Tensor], torch.Tensor]:
        ...
        if self.data["mask"] is None:
            ...
            mask = self.data["mask"][index.index_start : index.index_end + 1].clone()
        ...
        self.target_raw0 = deepcopy(target) 
        ...
        if self.randomize_length is not None:  # randomization improves generalization
            ...
            if new_encoder_length + new_decoder_length < len(target[0]):
                ...
                if mask is not None:
                    mask = mask[encoder_length - new_encoder_length : encoder_length + new_decoder_length]                    
        ...
        # zfbi
        if mask is not None:
            encoder_mask = mask[:encoder_length].to(torch.long)
            decoder_mask = mask[encoder_length:].to(torch.long)
        else:
            encoder_mask = torch.ones(encoder_length, dtype=torch.long)
            decoder_mask = torch.ones(decoder_length, dtype=torch.long)
        ...
        return (
            dict(
                ...
                groups=groups,
                target_scale=target_scale,
                encoder_mask=encoder_mask, # zfbi
                decoder_mask=decoder_mask, # zfbi
            ),
            (target, weight),
        )
    ...
    def _collate_fn(
        ...
        # zfbi
        encoder_mask = rnn.pad_sequence(
            [batch[0]["encoder_mask"] for batch in batches], batch_first=True
        )
        decoder_mask = rnn.pad_sequence(
            [batch[0]["decoder_mask"] for batch in batches], batch_first=True
        )
