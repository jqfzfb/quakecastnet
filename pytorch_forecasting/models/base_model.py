...
class PredictCallback(BasePredictionWriter):
    ...
    def on_predict_batch_end(
        ...
        outputs_cpu_dict = {key: value.cpu() if isinstance(value, torch.Tensor) and value.is_cuda else value
                           for key, value in outputs_dict.items()}
        out = type(outputs)(**outputs_cpu_dict)
...
class BaseModel(InitialParameterRepresenterMixIn, LightningModule, TupleOutputMixIn):
    ...
    def deduce_default_output_parameters(
        ...
        def get_output_size(normalizer, loss):
            if isinstance(loss, (QuantileLoss, QuantileMaskLoss)):
    ...
    def create_log(
        ...
        # zfbi
        if isinstance(self.loss, KLMSELoss):
            if self.log_interval > 0:
                kl_loss = out.get("kl_loss", None)
                if kl_loss is not None:
                    kl_loss_mean = kl_loss.detach().mean()
                    self.log("val_kl_loss", kl_loss_mean, on_epoch=True, prog_bar=True)
        
                pred = out["prediction"]
                target = torch.stack(y[0], dim=-1)
                mse = torch.mean((pred - target) ** 2).detach()
    ...
    def step(
        ...
        if self.training and len(self.hparams.monotone_constaints) > 0:
            ...
            if not self.predicting:
                ...
                elif isinstance(self.loss, (QuantileMaskLoss)):
                    kwargs = dict(
                        mask=x["decoder_mask"],
                    )
                elif isinstance(self.loss, KLMSELoss):
                    kwargs = dict(
                        kl_loss=out["kl_loss"], 
                    )
                else:
            ...
            if not self.predicting:
                if isinstance(self.loss, (MASE, MultiLoss)):
                    ...
                    kwargs = dict()
                loss = self.loss(prediction, y, **kwargs) 
            else:
                loss = None
        ...
        if stage != "predict" and loss is not None:
            self.log(
                f"{stage}_loss",
                loss,
                on_step=self.training,
    ...
    def _stage_name(self) -> str:
        ...
        if self.training:
            return "train"
        if t is not None:
            if getattr(t, "validating", False) or getattr(t, "sanity_checking", False):
                return "val"
            if getattr(t, "testing", False):
                return "test"
            if getattr(t, "predicting", False):
                return "predict"
        return "val"
    ...
    def predicting(self) -> bool:
        t = getattr(self, "trainer", None)
        return bool(t is not None and getattr(t, "predicting", False))
    ...
    def from_dataset(cls, dataset: TimeSeriesDataSet, **kwargs) -> LightningModule:
        ...
        if dataset.multi_target:
            if not isinstance(net.loss, KLMSELoss):
                assert isinstance(
                    net.loss, MultiLoss
