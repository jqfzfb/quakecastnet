...
class GATConvBlock(nn.Module):
    def __init__(self, input_dim, output_dim, heads=4, dropout=0.1, concat=True):
        super().__init__()
        self.gat = GATConv(
            in_channels=input_dim,
            out_channels=output_dim // heads if concat else output_dim,
            heads=heads,
            dropout=dropout,
            concat=concat,
            add_self_loops=True,
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        total_nodes = B * T * N
        batch_index = torch.arange(B * T).repeat_interleave(N).to(x.device)
        ...
        edge_index_big = torch.cat(edge_index_list, dim=1)  # shape: (2, B*T*E)
        edge_attr_big = torch.cat(edge_attr_list, dim=0)    # shape: (B*T*E, 1)

        # GAT forward
        h = self.gat(x, edge_index_big, edge_attr_big)
        h = self.activation(h)
        h = h.reshape(B * T, N, -1)
